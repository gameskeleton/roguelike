...
# @signal
# @impure
func _on_life_points_damage_taken(_damage: float, life_points: float, source: Object, _instigator: Object):
	_reset()
	...
	if life_points <= 0.0:
		for i in 10:
			...
			RkMain.get_main_node(self).current_room_node.add_child(pickup_node)
		queue_free()
