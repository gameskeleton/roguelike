...
func spawn(parent_node: Node, global_position: Vector2) -> Node:
	return RkObjectSpawner.spawn_coin(parent_node, global_position + Vector2(0, -8.0)).fly(expulse_direction, expulse_cone, expulse_strength)

# @override
# @impure
func spawn_preview(parent_node: Node, global_position: Vector2) -> Node:
	return RkObjectSpawner.spawn_coin(parent_node, global_position + Vector2(0, -8.0))
