...
func start_state() -> RkStateMachineState:
	...
	player_node.animation.play_animation(&"wall_slide")
...
func process_state(delta: float) -> RkStateMachineState:
	...
	player_node.movement.apply_gravity(delta, player_node.WALL_SLIDE_GRAVITY_MAX_SPEED, player_node.WALL_SLIDE_GRAVITY_ACCELERATION)
...
func finish_state() -> void:
	...
	if player_node.fsm.is_next_state_node([player_node.fsm.state_nodes.stand]) and player_node.input.velocity.x == 0:
		player_node.set_direction(-player_node.direction)
