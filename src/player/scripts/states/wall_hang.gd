...
func process_state(_delta: float):
	...
	if player_node.input_down.is_pressed():
		...
		if player_node.has_same_direction(player_node.direction, player_node.input_velocity.x) and player_node.is_able_to_wall_slide():
			return player_node.fsm.state_nodes.wall_slide
		return player_node.fsm.state_nodes.fall
	if player_node.input_jump.is_pressed() and player_node.is_able_to_jump() and player_node.has_invert_direction(player_node.direction, player_node.input_velocity.x):
